...
export default function ModalPublicacion({ isOpen, onClose, onSuccess }) {
  ...
  return (
    <>
      ...
      <MapPickerModal
        ...
        initialCoords={{ lat: formData.latitud, lng: formData.longitud }}
        onConfirm={({ latitud, longitud, direccion }) => {
          handleChange('latitud', latitud);
          handleChange('longitud', longitud);
          if (direccion) handleChange('ubicacion', direccion);
        }}
      />
    </>
  );
