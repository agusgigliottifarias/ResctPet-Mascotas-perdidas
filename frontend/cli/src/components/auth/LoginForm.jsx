...
export default function LoginForm({ onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Integración con el backend (LoginRequest DTO)
  ...
  return (
    <form onSubmit={handleSubmit} className="flex h-full w-full flex-col items-center justify-center p-10">
      <h3 className="text-2xl font-black mb-1 text-[#2D3748] tracking-tight">Iniciar Sesión</h3>
      ...
      <input
        ...
        className="mb-4 w-full rounded-2xl bg-white/60 backdrop-blur-md border border-white/90 px-4 py-2.5 text-sm text-[#2D3748] outline-none transition-all duration-200 focus:bg-white/90 focus:-translate-y-0.5 focus:shadow-[0_8px_20px_rgba(255,122,89,0.15)] focus:border-[#FF7A59]"
      />
      ...
      >
        {loading ? 'Ingresando...' : 'Entrar a Yirando'}
      </button>
    </form>
  );
}